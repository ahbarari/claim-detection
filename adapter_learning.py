...
class AdapterTransformerModel:
    ...
    def __init__(self, pretrained_model, task_name):
    ...
    def train(self, train_data,
        ...
        trainer.train()
        model.save_adapter(model_path, task_name)
        del model
        del trainer
    ...
    def test(self, test_data, model_path, result_output, random_seed):
        ...
        model.load_adapter(str(model_path), f'{str(model_path)}/adapter_config.json', set_active=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_claim", type=str)
    ...
    model = AdapterTransformerModel(pretrained_model=args.pretrained_model, task_name=args.task_name)
    train_dataset_dir = Path(args.train_dataset_dir)

    if args.train:
        train_claim = args.train_claim

        train_data = pd.read_csv(train_dataset_dir / f'{train_claim}_train.tsv', sep='\t')
        logger.info(f'Number of the training set: {len(train_data)}')

        val_data = pd.read_csv(train_dataset_dir / f'{train_claim}_dev.tsv', sep='\t', quoting=csv.QUOTE_NONE)
